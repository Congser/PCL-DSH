...
#Region "基础"
    ...
    Public Sub New()
        ...
        '窗体参数初始化
        FrmMain = Me
        '── PCL_DSH：启动页改为 DSH 控制台 ──
        '保留 PageLaunchLeft / PageLaunchRight 的实例（其他代码仍会引用它们），
        '但默认展示的是 DSH 的左右页面。
        FrmLaunchLeft = New PageLaunchLeft
        FrmLaunchRight = New PageLaunchRight
        FrmDshLeft = New PageDSHLeft
        '版本号改变
        ...
        End If
        '切换到首页
        '── PCL_DSH：首页改为 PageDSH（左栏 + 控制台），不再默认进入 MC 启动页 ──
        If Not IsNothing(FrmDshLeft.Parent) Then FrmDshLeft.SetValue(ContentPresenter.ContentProperty, Nothing)
        PanMainLeft.Child = FrmDshLeft
        PageLeft = FrmDshLeft
        PanMainRight.Child = FrmDshLeft.PageGet(PageSubType.DshOnline)
        PageRight = FrmDshLeft.PageGet(PageSubType.DshOnline)
        PageRight.PageState = MyPageRight.PageStates.ContentStay
        '模式提醒
    ...
    Private Sub FormMain_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        AniStart()
        TimerMainStart()
        'PCL_DSH：回收上次异常退出遗留的 dsh 进程。
        'dsh 是独立进程，PCL 被强杀时它不会跟着走；残留的进程会占着端口，
        '导致用户下次点启动时看到一屏 EADDRINUSE 堆栈。
        '放在后台线程做，不拖慢窗口显示 —— WMI 查询要几十到几百毫秒。
        RunInNewThread(
        Sub()
            Try
                DshService.ReclaimOrphanProcesses()
            Catch ex As Exception
                Logger.Warn($"DSH：启动时回收孤儿进程失败（可忽略）：{ex.Message}")
            End Try
        End Sub, "DSH 孤儿进程回收")
        '加载池
        RunInNewThread(
        Sub()
            ...
            End If
            '启动加载器池
            '── PCL_DSH：这里原本会拉起三个与 Minecraft 强相关的加载器 ──
            '
            '  JavaInit()                 扫全盘 Java（实测扫出 5 个，日志一大片）
            '  DlClientListMojangLoader   拉 Mojang 版本清单（一次外部 HTTP）
            '  ServerLoader               拉 MC 联机服务器列表
            '
            '本改版已把 MC 功能整体剥离：不启动游戏、不需要 Java、不做版本管理，
            '所以这三项的输出**没有任何消费者**，纯粹是启动时的浪费：
            '  · JavaInit 要遍历磁盘目录 + 起子进程跑 java -version（实测数百毫秒）
            '  · 两个 Loader 各发一次外部请求（实测 300~450ms，且要联网）
            '
            '全部去掉。`ModJava` 等 MC 模块的**源码保留**（靠 .vbproj 的 DSH 配置排除），
            '只是启动流程不再调用它们 —— 保留源码是为了将来 merge 上游时冲突最小。
            Try
                Thread.Sleep(100)
                RunCountSub()
            ...
            Telemetry("启动")
            '开源版本提示
            '── PCL_DSH：文案已按本改版的实际情况重写 ──
            ' 原版的说明列的是「CurseForge / 正版登录 / 百宝箱」这些 MC 侧的缺失项，
            ' 但本改版已经把 MC 功能整体剥离、并把「百宝箱」换成了「网络工具箱」，
            ' 照搬原版会让用户对着一条条已经不适用的说明发懵。
            If VersionBranchMain = "OpenSource" Then
                MyMsgBox(
                    "本程序基于 PCL 的源码改造，剥离了 Minecraft 功能，" &
                    "改造成 DeepSeek Harness（dsh）的启动与管理工具。" & vbCrLf & vbCrLf &
                    "与原版 PCL 的差异：" & vbCrLf &
                    "- 已移除：Minecraft 下载、启动、版本管理、正版登录、CurseForge 等全部 MC 功能" & vbCrLf &
                    "- 已移除：主题切换（需要赞助解锁的纪念性质功能）" & vbCrLf &
                    "- 已移除：更新与联网通知（避免滥用隐患）" & vbCrLf &
                    "- 已替换：原「百宝箱」→「网络工具箱」（GitHub 加速、IP 优选、网络诊断）" & vbCrLf &
                    "- 已新增：DSH 运行时管理、多实例、模型接入方式、插件市场" & vbCrLf & vbCrLf &
                    "本程序遵循 PCL 的开源许可，显示名以 Plain Craft Launcher 开头。",
                    "开源版本说明")
            End If
...
#Region "自定义窗口"
    ...
    Public Sub EndProgram(SendWarning As Boolean)
        'PCL_DSH：异步启动 dsh 清理。
        '这里只负责"把活儿派出去"，绝不等待 —— 真正停服务要 8~10 秒，
        '同步等就会让关闭动画僵住（用户报障的卡顿 10 秒）。
        '等待发生在 EndProgramForce 的最后，且设了 3 秒上限。
        DshShutdown()
        '强行结束下载任务？
    ...
    ''' <summary>
    ''' PCL_DSH：程序退出前的 DSH 清理。**后台线程执行，不阻塞关闭动画。**
    '''
    ''' 为什么必须异步：
    ''' <see cref="DshService.StopService"/> 会 <c>WaitForExit(800)</c> 再走 <c>taskkill</c>
    ''' 杀进程树。早期版本在 UI 线程同步调用它，
    ''' 用户点关闭后窗口要僵住数秒才消失（实测报障）。
    '''
    ''' 现在的做法是「**先放行关闭，再后台收尾**」：
    '''   1. 立刻记下要清理的东西
    '''   2. 丢到后台线程去停服务
    '''   3. 主线程照常走关闭动画并退出
    ''' dsh 进程是独立进程，父进程退出不会带走它，所以后台线程必须在
    ''' 主进程被 Kill 之前完成回收 —— 用 <see cref="WaitForDshCleanup"/> 给它一点时间。
    '''
    ''' ⚠️ 多实例改造后这里必须**逐个**检查：早期版本只要「主实例没在跑」就直接返回，
    ''' 结果第 2~4 个实例的 node 进程会全部变成孤儿。
    ''' </summary>
    Private Shared DshCleanupTask As Threading.Tasks.Task = Nothing

    Public Shared Sub DshShutdown()
        Try
            '只要还有**任何一个**实例有活着的进程，就得清理
            Dim alive As Integer = 0
            For Each inst In ModDSH.DshInstances
                If inst.HasLiveProcess OrElse inst.State <> ModDSH.DshState.Stopped Then alive += 1
            Next

            If alive = 0 Then Return

            Logger.Info($"DSH：程序退出，开始清理 {alive} 个 dsh 实例（后台执行）")

            '丢到后台线程去停，绝不阻塞 UI 线程
            DshCleanupTask = Threading.Tasks.Task.Run(
                Sub()
                    Try
                        DshService.StopAll()
                        Logger.Info("DSH：后台清理完成")
                    Catch ex As Exception
                        Logger.Warn($"DSH：后台清理 dsh 时出现异常（可忽略）：{ex.Message}")
                    End Try
                End Sub)
        Catch ex As Exception
            Logger.Warn($"DSH：启动退出清理时出现异常（可忽略）：{ex.Message}")
        End Try
    End Sub

    ''' <summary>
    ''' 等待后台的 dsh 清理完成，最多 <paramref name="TimeoutMs"/> 毫秒。
    ''' </summary>
    ''' <remarks>
    ''' 由 <see cref="EndProgramForce"/> 在真正退出前调用。
    ''' 这个等待有**上限**（默认 3 秒），保证即使 dsh 卡住也不会把用户永久留住；
    ''' 超时就放弃等待，进程照常退出（残留的 node 由下次启动时的端口探测处理）。
    ''' </remarks>
    Public Shared Sub WaitForDshCleanup(Optional TimeoutMs As Integer = -1)
        Try
            If TimeoutMs < 0 Then TimeoutMs = ModDSH.DshShutdownWaitMs
            If DshCleanupTask Is Nothing Then Return
            If DshCleanupTask.Wait(TimeoutMs) Then
                Logger.Info($"DSH：清理已在 {TimeoutMs} ms 内完成")
            Else
                Logger.Warn($"DSH：清理超过 {TimeoutMs} ms 仍未完成，放弃等待直接退出")
            End If
        Catch ex As Exception
            Logger.Warn($"DSH：等待清理时出现异常（可忽略）：{ex.Message}")
        End Try
    End Sub
    ...
    Public Shared Sub EndProgramForce(Optional ReturnCode As ProcessReturnValues = ProcessReturnValues.Success)
        ...
        ConfigUtils.SaveAll()
        'PCL_DSH：给后台的 dsh 清理一点时间。必须在 Flush 之前等，
        '否则清理日志还没写进文件，进程就被 Kill 掉了。
        WaitForDshCleanup()
...
#Region "窗体事件"
    ...
    Private Sub FormMain_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        ...
        End If
        '调用启动游戏
        '── PCL_DSH：主页已不再是 MC 启动页，回车键不再触发启动游戏 ──
        '原逻辑：If e.Key = Key.Enter AndAlso PageCurrent = FormMain.PageType.Launch Then
        '            If IsAprilEnabled AndAlso Not IsAprilGiveup Then Hint("木大!") Else FrmLaunchLeft.LaunchButtonClick()
        '        End If
        '保留判断体但永不触发，方便后续若恢复 MC 启动页时快速还原。
        If False AndAlso e.Key = Key.Enter AndAlso PageCurrent = FormMain.PageType.Launch Then
    ...
    Private Sub TriggerPageBack()
        '⚠️ 必须带 FrmDownloadInstall IsNot Nothing：
        '   PCL_DSH 里 PageSubType.DownloadInstall 与 DownloadMirror 共用数值 0，
        '   而镜像版本页**不会**创建 FrmDownloadInstall，
        '   少了这个判空就会在打开下载页时直接空引用崩溃。
        If PageCurrent = PageType.Download AndAlso PageCurrentSub = PageSubType.DownloadInstall AndAlso
           FrmDownloadInstall IsNot Nothing AndAlso FrmDownloadInstall.IsInSelectPage Then
                ...
                Try
                    ...
                    If Str.StartsWithF("authlib-injector:yggdrasil-server:") Then
                        ...
                        ElseIf PageCurrent = PageType.Launch Then
                            '正在主页，需要刷新左栏
                            '── PCL_DSH：主页左栏已换成 PageDSHLeft ──
                            If FrmDshLeft IsNot Nothing Then FrmDshLeft.Refresh()
                        End If
                    ElseIf Str.StartsWithF("file:///") Then
                        '文件拖拽（例如从浏览器下载窗口拖入）
    ...
    Private Sub FileDrag(FilePathList As IEnumerable(Of String))
        ...
        Sub()
            ...
            End If
            ' ── PCL_DSH：dsh 运行时包导入 ──
            ' ⚠️ 必须放在「安装整合包」**之前**。否则拖进来的 zip 会先被 MC 的
            '   整合包逻辑（ModpackInstall）接走，用户永远等不到运行时导入。
            '   判断只看 zip 的条目名（不解压），所以哪怕是个 200 MB 的包也是秒回。
            If DshRuntimeSlot.DshSlotLooksLikeRuntimeZip(FilePath) Then
                Logger.Info($"拖入的压缩包里有 dsh 运行时入口，按运行时包处理：{FilePath}")
                Hint("检测到 dsh 运行时包，正在校验……")
                RunInUi(
                    Sub()
                        Try
                            ' 先把页面准备好 —— 导入进度是显示在这个页面上的
                            If FrmDownloadMirror Is Nothing Then FrmDownloadMirror = New PageDownloadMirror
                            FrmMain.PageChange(FormMain.PageType.Download, FormMain.PageSubType.DownloadMirror)
                            FrmDownloadMirror.ImportRuntimeFromPath(FilePath)
                        Catch ex As Exception
                            Logger.Error(ex, "DSH：启动运行时导入失败")
                            Hint($"无法导入这个包：{ex.Message}", HintType.Red)
                        End Try
                    End Sub)
                Return
            End If
...
#Region "切换页面"
    ...
    Public Enum PageType
        ...
        HelpDetail = 9
        ''' <summary>
        ''' PCL_DSH：运行日志。从顶部菜单栏的「日志」按钮进入。
        ''' </summary>
        DSHLog = 10
        ''' <summary>
        ''' PCL_DSH：实例快照。从顶部菜单栏的「快照」按钮进入。
        ''' </summary>
        Snapshot = 11
    End Enum
    ''' <summary>
    ''' 次要页面种类。
    '''
    ''' ⚠️ 历史上这个枚举的数值必须等于左栏 StackPanel 里的**下标**
    ''' （因为 <see cref="PageChange"/> 用 <c>PanItem.Children(SubType)</c> 取控件）。
    ''' 现在改成**按 Tag 查找**（见 <see cref="FindListItemByTag"/>），下标不再有约束，
    ''' 但为了让既有的 Tag 值继续有效，这里的数值约定保持不变：
    ''' 左栏每个 MyListItem 的 <c>Tag</c> 应当等于它对应的枚举值。
    '''
    ''' 该枚举在自定义事件中使用，是公开 API 的一部分。
    ''' </summary>
    Public Enum PageSubType
        [Default] = 0
        '── PCL_DSH：下载页已整体替换为「镜像版本」+「插件社区」 ──
        ' 0 号槽位现在是镜像版本页；DownloadInstall 保留为**别名**，
        ' 让仍在跳「安装新版本」的 MC 侧代码不至于编译失败（会落到镜像版本页）。
        DownloadMirror = 0
        DownloadInstall = 0
        DownloadPlugin = 1
        '以下 5 个是原版的社区资源子页面。PCL_DSH 已把它们的入口从左栏移除，
        '但类本身还在（MC 侧的若干跳转仍会引用），所以枚举值一并保留。
        DownloadMod = 2
        DownloadPack = 3
        DownloadDataPack = 4
        ...
        InstanceExport = 4
        '── PCL_DSH：DSH 左栏不再有子页面导航（方案A）。
        ' 多实例改造后左栏是「我的实例」，右栏固定为「在线模式配置」：
        '   DshOnline — 启动页右栏（原 DshConsole，控制台内容已搬到「日志」菜单）
        ' 原来的 DshAbout / DshLog / DshEnv 已下线：
        '   关于   → 并入「设置 → 关于」
        '   日志   → 顶部菜单栏「日志」按钮（PageType.DSHLog）
        '   环境   → 并入「设置 → 启动」
        ' 这里只留一个 DshOnline 指向 0，兼容旧代码里对它的引用。
        DshOnline = 0
    End Enum
    ''' <summary>
    ...
    ''' <summary>
    ''' 在左栏的 StackPanel 里按 <c>Tag</c> 找到对应的 <see cref="MyListItem"/>。
    ''' </summary>
    ''' <param name="Pan">左栏的 StackPanel（如 <c>FrmDownloadLeft.PanItem</c>）。</param>
    ''' <param name="Tag">目标 <see cref="PageSubType"/> 值。</param>
    ''' <returns>找到的条目；找不到返回 Nothing。</returns>
    ''' <remarks>
    ''' 原实现是 <c>PanItem.Children(SubType)</c> —— 直接按下标取。
    ''' 那要求「枚举值 == StackPanel 下标」，于是左栏里连一个分隔用的 TextBlock
    ''' 都会把后面所有条目的下标顶偏（原版下载页的 <c>DownloadMod = 2</c> 就是这么来的）。
    ''' 更糟的是**越界或取到非 MyListItem 时直接抛异常**，一改左栏就得回头核对枚举。
    '''
    ''' 改成按 Tag 查找后：
    '''   - 左栏增删条目不再影响枚举
    '''   - 找不到只是"没有可高亮的项"，不会崩
    ''' 每个 MyListItem 的 Tag 约定等于它对应的 PageSubType 值。
    ''' </remarks>
    Private Shared Function FindListItemByTag(Pan As Panel, Tag As Integer) As MyListItem
        If Pan Is Nothing Then Return Nothing
        For Each Child As Object In Pan.Children
            Dim Item As MyListItem = TryCast(Child, MyListItem)
            If Item Is Nothing OrElse Item.Tag Is Nothing Then Continue For
            Dim Value As Integer
            If Integer.TryParse(Item.Tag.ToString(), Value) AndAlso Value = Tag Then Return Item
        Next
        Return Nothing
    End Function
    ...
    Public Sub PageChange(Stack As PageStackData, Optional SubType As PageSubType = PageSubType.Default)
        If PageNameGet(Stack) = "" Then
            ...
            Select Case Stack.Page
                Case PageType.Download
                    If FrmDownloadLeft Is Nothing Then FrmDownloadLeft = New PageDownloadLeft
                    FindListItemByTag(FrmDownloadLeft.PanItem, SubType)?.SetChecked(True, True, Stack = PageCurrent)
                Case PageType.Setup
                    If FrmSetupLeft Is Nothing Then FrmSetupLeft = New PageSetupLeft
                    FindListItemByTag(FrmSetupLeft.PanItem, SubType)?.SetChecked(True, True, Stack = PageCurrent)
                Case PageType.Other
                    If FrmOtherLeft Is Nothing Then FrmOtherLeft = New PageOtherLeft
                    FindListItemByTag(FrmOtherLeft.PanItem, SubType)?.SetChecked(True, True, Stack = PageCurrent)
            End Select
        ...
        Else
            ...
            Select Case Stack.Page
                Case PageType.InstanceSetup
                    If FrmInstanceLeft Is Nothing Then FrmInstanceLeft = New PageInstanceLeft
                    FindListItemByTag(FrmInstanceLeft.PanItem, SubType)?.SetChecked(True, True, Stack = PageCurrent)
            End Select
            PageChangeActual(Stack, SubType)
        End If
    End Sub
    ...
    ''' 通过点击导航栏改变页面。
    ''' </summary>
    Private Sub BtnTitleSelect_Click(sender As MyRadioButton, raiseByMouse As Boolean) Handles BtnTitleSelect0.Check, BtnTitleSelect1.Check, BtnTitleSelect2.Check, BtnTitleSelect3.Check, BtnTitleSelect4.Check, BtnTitleSelect5.Check, BtnTitleSelect6.Check
...
#Region "实际更改页面框架 UI"
            ...
            PageCurrent = Stack
            Select Case Stack.Page
                Case PageType.Launch '启动 → PCL_DSH：改为 DSH 页面
                    If FrmDshLeft Is Nothing Then FrmDshLeft = New PageDSHLeft
                    PageChangeAnim(FrmDshLeft, FrmDshLeft.PageGet(SubType))
                ...
                Case PageType.DSHLog 'PCL_DSH：运行日志
                    '日志是"独立页面"而不是 DSH 的子页面 —— 顶部菜单栏直接切过来，
                    '左栏留空（日志自己占满右栏）。
                    If FrmDshLog Is Nothing Then FrmDshLog = New PageDSHLog
                    PageChangeAnim(New MyPageLeft, FrmDshLog)
                Case PageType.Snapshot 'PCL_DSH：实例快照
                    '与日志页同样处理：独立页面，左栏留空。
                    If FrmSnapshot Is Nothing Then FrmSnapshot = New PageSnapshot
                    PageChangeAnim(New MyPageLeft, FrmSnapshot)
            End Select
#End Region
