...
#Region "加载器"
    ...
    Public Sub PageLoaderInit(Of TIn, TOut)(LoaderUi As MyLoading, PanLoader As FrameworkElement, PanContent As FrameworkElement, PanAlways As FrameworkElement,
        ...
        If FinishedInvoke IsNot Nothing Then
            ...
            Sub()
                '── PCL_DSH：诊断等待循环 ──
                ' 下面这个循环会**无限自旋**在 PageExit / ContentExit 状态上，
                ' 因为它跑在加载器的后台线程，而页面退出动画需要 UI 线程推进。
                ' 一旦用户在加载期间切走页面，FinishedInvoke 就永远不会被执行。
                ' PCL 原版用 Trace 记录，出问题时看不到，所以这里保留一条 Warn。
                Dim WasBlocked As Boolean =
                    PageState = MyPageRight.PageStates.PageExit OrElse PageState = MyPageRight.PageStates.ContentExit
                If WasBlocked Then
                    Logger.Warn($"DSH：加载器完成回调被页面退出状态阻塞，等待中（状态 {PageState}）")
                End If
                ...
                Loop
                If WasBlocked Then Logger.Info("DSH：页面退出状态已结束，继续执行加载器完成回调")
