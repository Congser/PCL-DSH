...
#Region "页面声明"
    ...
    '── PCL_DSH：下载页已换成 dsh 自己的两个子页面 ──
    Public FrmDownloadMirror As PageDownloadMirror
    Public FrmDownloadPlugin As PageDownloadPlugin
    '以下 6 个是原版的 Minecraft 社区资源页面。入口已从左栏移除，
    '但类保留着（MC 侧仍有跳转引用），按需创建。
    ...
    '── PCL_DSH：DSH 页面声明 ──
    '启动页左栏是「我的实例」，右栏是「在线模式配置」（原 DSH 控制台页）。
    '原控制台的三张卡片已按需求搬进 PageDSHLog（顶部菜单栏「日志」进入）。
    Public FrmDshLeft As PageDSHLeft
    Public FrmDshOnline As PageDSHOnline
    Public FrmDshLog As PageDSHLog
    ''' <summary>PCL_DSH：实例快照页（顶部菜单栏「快照」）。</summary>
    Public FrmSnapshot As PageSnapshot
    Public FrmDshEnv As PageDSHEnv
